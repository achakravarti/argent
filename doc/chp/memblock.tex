...
Sometimes client code requires to resize an existing block of heap memory, even
though this is a rare operation within the implementation of the Argent Library.
In order to faciliate this, the interface function \agMemblockResize has \\
been provided.

The \agMemblockResize function takes two arguments, the first being a handle to
the memory block that needs to be resized, and the second being the new size.
The first argument is required to be a valid handle (a non-null double pointer
to a block of heap memory, and the second argument is required to be greater
than zero. The size specified by the second argument may be less than the
original size of the memory block.
...
Note that, as in the case of \agMemblockNew, if \agMemblockResize is successful,
then the new size of the memory block passed as the first argument may be larger
than the amount specified by the second argument owing to alignment 
restrictions.

In case \agMemblockResize fails to resize the memory block, then it raises the
\texttt{AG\_ERNO\_MEMBLOCK\_RESIZE} error code, which is subsequently passed to 
the exception handler (defined by client code) to process. 

The algorithm for \agMemblockResize is given by Algorithm
\ref{alg:agMemblockResize}. We assert that the block of heap memory to resize is
valid, and that new size is greater than zero. We then call the standard C
library function \texttt{realloc()} to resize the memory buffer and ensure that
the resized buffer is valid.
