...
/**
 * The ag_http_response testing interface provides the following five macros for
 * generating test cases specific to the ag_http_response interface:
 *   - AG_METATEST_HTTP_RESPONSE_HEADER()  : for ag_http_response_header()
 *   - AG_METATEST_HTTP_RESPONSE_BODY()    : for ag_http_response_body()
 *   - AG_METATEST_HTTP_RESPONSE_ADD()     : for ag_http_response_add()
 *   - AG_METATEST_HTTP_RESPONSE_ADD_FILE(): for ag_http_response_add_file()
 *   - AG_METATEST_HTTP_RESPONSE_FLUSH()   : for ag_http_response_flush()
 *
 * All of the above functions require that the argument to the first parameter
 * be the sample HTTP response instance returned by a function generated by the 
 * AG_SAMPLE_HTTP_RESPONSE() macro (or its overloaded forms).
 *
 * In the case of the first two metatest macros above, the string value expected
 * to be returned by the tested interface function is passed through the second
 * parameter. In the case of AG_METATEST_HTTP_RESPONSE_ADD() and its overloaded
 * form, the expected new value of the body of the sample is passed through the
 * **third** parameter.
 *
 * The third parameter of AG_METATEST_HTTP_RESPONSE_ADD() and its is overloaded
 * form is, respectively, the string content and the path of the file content
 * that is to be added to the body of the sample HTTP response instance.
 *
 * AG_HTTP_RESPONSE_FLUSH() does not have any other parameters, since the
 * expected result of calling ag_http_response_flush() is to empty the body of
 * an HTTP response instance.
 **/
